...
class BotUtils:
    ...
    def is_admin(self, user: discord.User):
        return user.id in [
            ...
            285108244714881024,  # aahoughton
        ]

    def is_gm(self, config: config.ServerConfig, member: discord.Member):
        role_checks = map(lambda r: config.is_gm_role(r.name), member.roles)
        return any(role_checks)

    def is_gm_channel(
        self, config: config.ServerConfig, channel: commands.Context.channel
    ) -> bool:
        return config.is_gm_channel(channel.name) and config.is_gm_category(
            channel.category.name
        )

    def is_impdip_member(self, member: discord.Member) -> bool:
        hub = self.bot.get_guild(config.IMPDIP_HUB_SERVER)
        if not hub:
            print(f"Can't find the hub server {config.IMPDIP_HUB_SERVER}")
            return False

        exists = discord.utils.find(lambda m: m.id == member.id, hub.members)
        if exists is None:
            return False

        return True

    def is_impdip_verified(self, member: discord.Member) -> bool:
        if not self.is_impdip_member(member):
            return False
        ...
        member_hub = discord.utils.find(lambda m: m.id == member.id, hub.members)
        if member_hub is None:
            return False

        if not discord.utils.find(
            lambda r: r.name == "ImpDip Verified", member_hub.roles
        ):
            return False
...
def get_player_by_channel(
    ...
    name = channel.name
    if (not ignore_catagory) and not board.config.is_player_category(
        channel.category.name
    ):
        return None
...
def get_orders(
    ...
    if phase.is_builds(board.phase):
        ...
        for player in sorted(players, key=lambda sort_player: sort_player.name):
...
def get_filtered_orders(board: Board, player_restriction: Player) -> str:
    ...
    for player in board.players:
        if phase.is_retreats(board.phase):
            ...
            in_moves = lambda _: True
        moving_units = [
            unit for unit in player.units if in_moves(unit) and unit.province in visible
        ]
