...
from bot.config import IMPDIP_SERVER_ID, ERROR_COLOUR
...
class SpecView(discord.ui.View):
    ...
    async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.manager.get_spec_request(guild.id, self.member.id):
            await interaction.response.send_message(
        ...
        resp = self.manager.save_spec_request(
            guild.id, self.member.id, self.power_role.id
        )
...
class SpectateCog(commands.Cog):
    def __init__(self, bot: DiploBot):
        self.bot = bot
        self.manager = bot.manager

    @discord.app_commands.command(name="spec", description="Specatate a Player")
    async def spec(self, interaction: discord.Interaction, power_role: discord.Role):
        ...
        prev_request = self.bot.manager.get_spec_request(guild.id, interaction.user.id)
        if prev_request:
        ...
        await role_channel.send(
            ...
            view=SpecView(
                ...
                member=requester,
                game_name=guild.name,
                admin_channel=admin_channel,
                channel_url=url,
                role=power_role,
                cspec_role=cspec_role,
            ),
        )
    ...
    async def record_spec(self, ctx: commands.Context):
        guild = ctx.guild
        if not guild:
            return
        ...
        out = self.manager.save_spec_request(guild.id, user_id, power_id)
    ...
    async def backlog_specs(self, ctx: commands.Context) -> None:
        ...
        cspec = discord.utils.find(lambda r: r.name == "Country Spectator", guild.roles)
        if cspec is None:
            await send_message_and_file(
                channel=ctx.channel,
                title="Error",
        ...
        for member in guild.members:
            ...
            result = self.manager.save_spec_request(
                guild.id, member.id, power_role.id
            )
...
async def setup(bot: DiploBot):
    await bot.add_cog(SpectateCog(bot))
