...
from bot.config import ERROR_COLOUR, IMPDIP_HUB_SERVER, IMPDIP_HUB_STATUS_CHANNEL
from bot.perms import admin_only, CommandPermissionError, gm_only
from bot.utils import BotUtils, send_message_and_file
...
class DiploGM(commands.Bot):
    ...
    async def announce_online(self):
        ...
        guild = self.get_guild(IMPDIP_HUB_SERVER)  # Get the impdip hub server
        if not guild:
            print(f"Guild with ID {IMPDIP_HUB_SERVER} not found.")
            return

        channel = self.get_channel(
            IMPDIP_HUB_STATUS_CHANNEL
        )  # Get the bot status channel
        if not channel:
            print(f"Channel with ID {IMPDIP_HUB_STATUS_CHANNEL} not found.")
            return
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        epoch = member.created_at
        since = datetime.datetime.now() - epoch

        if since.total_days() < 7:
            pass
...
#     brief="wipe",
# )
# async def wipe(ctx: commands.Context) -> None:#     await command.wipe(ctx, manager)
#
#
# @bot.command(
